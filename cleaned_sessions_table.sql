...
-- replacing blank values in end_time to null

UPDATE sessions
SET end_time = NULL
WHERE end_time = '';

-- Replacing null end_time values with average time difference

SELECT SEC_TO_TIME(AVG(TIMESTAMPDIFF(SECOND, start_time, end_time))) AS avg_time_difference
FROM sessions;

-- The avg time difference is 00:32:59 and now we will replce the end_time with start_time + avg_time_difference 
